...
for(const t of tasks)for(const s of Object.values(t.predictions)){
 const original=G.parse(s.d);assert.equal(original.nodes.length,s.original_anchor_count);assert.equal(original.edges.length,s.original_anchor_count);
 ...
 const g=clone(original);let n=g.nodes.findIndex((_,i)=>g.edges.find(e=>e.b===i)?.c2&&g.edges.find(e=>e.a===i)?.c1);
 if(n<0){n=0;const inc=g.edges.find(e=>e.b===n);G.curve(g,inc.a);G.curve(g,n)}
 G.smooth(g,n);const e=g.edges.findIndex(e=>e.a===n);G.moveControl(g,e,'c1',{x:g.nodes[n].x+10,y:g.nodes[n].y+20});let inc=g.edges.find(e=>e.b===n),p=g.nodes[n],q=g.edges[e].c1,r=inc.c2;assert(Math.abs((q.x-p.x)*(r.y-p.y)-(q.y-p.y)*(r.x-p.x))<1e-6);
 cases++;
}
